...
public class ProStatisticsExcelFileGenerator {
	private ArrayList<String> fieldName = null; // excel标题数据集

	private List<ProStatisticsBean>   fieldData = null; // excel数据内容,导出统计信息
	...
	public ProStatisticsExcelFileGenerator(ArrayList<String> fieldName,
			List<ProStatisticsBean> fieldData) {

		this.fieldName = fieldName;
		this.fieldData = fieldData;
	}

	public ProStatisticsExcelFileGenerator() {
		super();
	}
	...
	public HSSFWorkbook createWorkbook() throws UnsupportedEncodingException {
		...
		workBook.setSheetName(0,"播放统计信息",HSSFWorkbook.ENCODING_UTF_16);//设置sheet名称的编码
		...
		for (int i = 0; i < fieldData.size(); i++) {
			...
			ProStatisticsBean proBean = fieldData.get(i);
			...
			cell0.setCellValue(proBean.getTerminalName());
			cell1.setCellValue(proBean.getProgramName());
			cell2.setCellValue(proBean.getProgramType());
			cell3.setCellValue(proBean.getProgramLevel());
			cell4.setCellValue(proBean.getTotalTime());
			cell5.setCellValue(proBean.getStartTime());
			cell6.setCellValue(proBean.getEndTime());
