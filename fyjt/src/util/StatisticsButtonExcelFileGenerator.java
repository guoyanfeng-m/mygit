...
public class StatisticsButtonExcelFileGenerator {
	private ArrayList<String> fieldName = null; // excel标题数据集

	private List<StatisticsButtonBean>   fieldData = null; // excel数据内容
	...
	public StatisticsButtonExcelFileGenerator(ArrayList<String> fieldName,
			List<StatisticsButtonBean> fieldData) {

		this.fieldName = fieldName;
		this.fieldData = fieldData;
	}

	public StatisticsButtonExcelFileGenerator() {
		super();
	}
	...
	public void createWorkBook(int counts,int page,int start,int end){
		...
		workBook.setSheetName(page-1,"触摸信息统计"+"_"+page,HSSFWorkbook.ENCODING_UTF_16);//设置sheet名称的编码
...
//		for (int i = 0; i < fieldData.size(); i++) {
		for (int i = start; i < end; i++) {
			...
			StatisticsButtonBean count = fieldData.get(i);
			...
			cell0.setCellValue(count.getTerminalid());
			cell1.setCellValue(count.getTerminalName());
			cell2.setCellValue(count.getProgramName());
			cell3.setCellValue(count.getSceneNameofButton());
			String val = count.getButtonType();
			String type = "";
			if(val.equals("0")){
				type = "无";
			...
			cell4.setCellValue(count.getButtonName());
			cell5.setCellValue(type);
			cell6.setCellValue(count.getSceneNameofJumpbutton());
			cell7.setCellValue(count.getClickTime().toString().substring(0, count.getClickTime().toString().indexOf(".")));
